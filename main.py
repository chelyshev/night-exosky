...
from calculations import apparent_to_absolute_magnitude, absolute_magnitude_to_pseudosize, convert_teff_to_rgb, \
    ...
    possition_angle_of_exo_pl_relative_to_stars
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.gaia_tbl = QTable.read('gaia_data_small.fits', format='fits')
        ...
        dataset_dropdown_list = QComboBox()
        dataset_list = ["small", "light", "big"]
        dataset_dropdown_list.addItems(dataset_list)

        pl_dropdown_list = QComboBox()
        pl_name = self.nasa_ps_tbl['pl_name']
        pl_dropdown_list.addItem("Earth")
        pl_dropdown_list.addItems(pl_name)

        # The default signal from currentIndexChanged sends the index
        pl_dropdown_list.currentIndexChanged.connect(self.index_changed)

        # The same signal can send a text string
        dataset_dropdown_list.currentTextChanged.connect(self.text_changed)

        self.setCentralWidget(pl_dropdown_list)
        ...
        layout.addWidget(dataset_dropdown_list)
        layout.addWidget(pl_dropdown_list)
    ...
    def text_changed(self, i):
        file_name = "gaia_data_" + i + ".fits"
        print("Stars dataset changed to: ", i)
        self.gaia_tbl = QTable.read(file_name, format='fits')
    ...
    def index_changed(self, i):
        ...
        if i == -1:
            ...
            magnitude = self.gaia_tbl['phot_g_mean_mag']
            abs_mag = apparent_to_absolute_magnitude(self.night_exosky_coords_galactic.distance.value, magnitude.value)
            self.pseudosize = absolute_magnitude_to_pseudosize(abs_mag)
            ...
            self.sc.axes.scatter(self.night_exosky_coords_galactic.l.degree, self.night_exosky_coords_galactic.b.degree,
                                 s=self.pseudosize, color=self.stellar_color,
                                 marker='.', linewidths=0)
            self.sc.axes.grid(ls='solid', lw=.1)
            self.sc.axes.set_xlim(0, 360)
            self.sc.axes.set_ylim(-90, 90)
            ...
            pl_dist = self.nasa_ps_tbl['sy_dist'].value
            ...
            self.sc.axes.scatter(self.night_exosky_coords_galactic.l.degree, self.night_exosky_coords_galactic.b.degree,
                                 ...
                                 marker='.', linewidths=0, edgecolors='face')
            self.sc.axes.grid(ls='solid', lw=.1)
            # self.update()
            # self.showFullScreen()
            self.sc.draw()
